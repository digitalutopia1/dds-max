...
class DDSClassDesc:public ClassDesc2 {
   ...
   public:
      ...
      Class_ID        ClassID      ( )                   { return Class_ID(0x5ff91659, 0x21ae3c81);    }
      const TCHAR    *Category     ( )                   { return GetString(IDS_DDS_IO); }
      const TCHAR    *InternalName ( )                   { return _T("ddsio"); }
...
static BitmapIO_Bmp_Imp bmpIOInterface(
      BMPIO_INTERFACE, _T("ibmpio"), IDS_DDSIO_INTERFACE, &DDSDesc, 0,
         BitmapIO_Bmp_Imp::bmpio_getType, _T("getType"), IDS_DDSIO_GETBITMAPTYPE, TYPE_ENUM, BitmapIO_Bmp_Imp::bmpio_type, 0, 0, 
         BitmapIO_Bmp_Imp::bmpio_setType, _T("setType"), IDS_DDSIO_SETBITMAPTYPE, TYPE_VOID, 0, 1, 
...
int BitmapIO_Bmp_Imp::GetType() {
   ...
   BitmapIO_DDS* p = new BitmapIO_DDS;
   if (p) {
      p->ReadCfg();
...
BitmapIO_DDS::BitmapIO_DDS( ) { 
   memset(&mParams,0,sizeof(DDSParams));
...
BOOL BitmapIO_DDS::LoadConfigure ( void *ptr, DWORD piDataSize ) {
...
INT_PTR BitmapIO_DDS::ConfigCtrlDlgProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam) {
    switch (message) {
      case WM_INITDIALOG:
         if (!mParams.saved)
            ReadCfg();
         switch (mParams.outDepth) {
            case BMM_PALETTED:
               CheckDlgButton( hWnd, IDC_DDS_PALETTE, TRUE );
               break;
            case BMM_NO_TYPE:
            case BMM_TRUE_24:
               CheckDlgButton( hWnd, IDC_DDS_RGB24, TRUE );
               break;
         }
         break;
      case WM_COMMAND:
...
INT_PTR CALLBACK StaticDialogProc(HWND hwnd, UINT msg, WPARAM w, LPARAM l) {
   ...
   if (msg==WM_INITDIALOG) {
      p = (BitmapIO_DDS*)l;
       ...
       if ( (p = DLGetWindowLongPtr<BitmapIO_DDS*>(hwnd) ) == NULL )
         return FALSE; 
   }
...
BMMRES BitmapIO_DDS::GetImageInfo ( BitmapInfo *fbi ) {
   ...
   fbi->SetWidth( (WORD)ddsd.dwWidth );
   fbi->SetHeight((WORD)ddsd.dwHeight);
   bmi.biBitCount = 32;
...
BitmapStorage *BitmapIO_DDS::Load(BitmapInfo *fbi, Bitmap *map, BMMRES *status) 
{
   ...
   *status = BMMRES_SUCCESS;
   ...
   ATI_TC_Texture texture2;

   fbi->SetWidth( (WORD)ddsd.dwWidth );
   fbi->SetHeight((WORD)ddsd.dwHeight);
   memset(&texture, 0, sizeof(texture));
   memset(&texture2, 0, sizeof(texture2));
   texture2.dwSize = sizeof(texture);
   texture2.dwWidth = ddsd.dwWidth;
   texture2.dwHeight = ddsd.dwHeight;
   texture2.dwPitch = ddsd.lPitch;
   
   
   if (ddsd.ddpfPixelFormat.dwRGBBitCount == 32)
	   texture2.format = ATI_TC_FORMAT_ARGB_8888;
   else if (ddsd.ddpfPixelFormat.dwRGBBitCount == 24)
	   texture2.format = ATI_TC_FORMAT_RGB_888;
   else if (GetFormat(ddsd.ddpfPixelFormat.dwPrivateFormatBitCount) != ATI_TC_FORMAT_Unknown)
	   texture2.format = GetFormat(ddsd.ddpfPixelFormat.dwPrivateFormatBitCount);
   else if (GetFormat(ddsd.ddpfPixelFormat.dwFourCC) != ATI_TC_FORMAT_Unknown)
	   texture2.format = GetFormat(ddsd.ddpfPixelFormat.dwFourCC);
   else
   {
   ...
   texture2.dwDataSize = ATI_TC_CalculateBufferSize(&texture2);
   texture2.pData = (ATI_TC_BYTE*)malloc(texture2.dwDataSize);

   fread(texture2.pData, texture2.dwDataSize, 1, file.stream);

   //What follows is simplified - primarily just to see if it works, but also because it really doesn't matter. 32 bit ARGB should handle everything. 


   ATI_TC_FORMAT destFormat = ATI_TC_FORMAT::ATI_TC_FORMAT_ARGB_8888;
   texture.dwSize = sizeof(texture);
   texture.dwWidth = texture2.dwWidth;
   texture.dwHeight = texture2.dwHeight;
   texture.dwPitch = 0;
   ...
   options.nCompressionSpeed = ATI_TC_Speed::ATI_TC_Speed_SuperFast;
   ATI_TC_ConvertTexture(&texture2, &texture, &options, NULL, NULL, NULL);
...
// fbi->SetGamma(1.0f);
   fbi->SetAspect(1.0f);
   fbi->SetType(BMM_TRUE_32); //again - default values of 32 bit ARGB
   fbi->SetFlags(MAP_HAS_ALPHA);	//even if it doesn't have alpha - it does now!
   bmi.biBitCount = 32;
      ...
      case 32:
         {
            ...
            BYTE *ptr;
			/*std::vector<std::vector<BYTE>> pData(h + 1);
			for (int i = 0; i < pData.size(); i++)
			{
				std::vector<BYTE> tempVec(w*4);
				memcpy(&tempVec[0], texture.pData + (i*w*4), w * 4);
				pData[i] = tempVec;
			}*/
			for (int i = 0; i <= h; i++)
			{
				//memcpy(p, &texture.pData + (i*wb), wb);
				//ptr = p;
				for (int x = 0; x < w; x++)
				{
					b[x].b = (WORD)((*texture.pData++) << 8);
					b[x].g = (WORD)((*texture.pData++) << 8);
					b[x].r = (WORD)((*texture.pData++) << 8);
					if (hasAlpha) {
						b[x].a = (WORD)((*texture.pData++) << 8);
					}
					
				}
				if (s->PutPixels(0, (i), w, b) != 1)
					goto io_error_out;
				if (fbi->GetUpdateWindow())
					SendMessage(fbi->GetUpdateWindow(), BMM_PROGRESS, i, h);
...
int  BitmapIO_DDS::ReadBimpHeader( FILE *stream ) {

   //-- Read File Header --------------------------------
   DWORD dwFileHeader;
   fread(&dwFileHeader, sizeof(DWORD), 1, stream);
...
int  BitmapIO_DDS::ReadDDSHeader(FILE *stream) {
	...
	size_t res = fread(&ddsd, sizeof(DDSD2), 1, stream);
...
void BitmapIO_DDS::GetCfgFilename( TCHAR *filename ) {
   ...
   _tcscat(filename,DDSCONFIGNAME);   
}
...
void BitmapIO_DDS::WriteCfg() {
   ...
   File file(filename, _T("wb"));
   if (file.stream) {
      fwrite(&tbuf,1,sizeof(DDSParams),file.stream);
   }
}
